...
def calculate_jaccard_score(gold_label, predict_label, type='token'):
    ''' calculate Jaccard score based on token or entity level
        parameters:
            gold_label, predict_label: gold and predict labels
            type: token level or entity level, entity level by default
        return:
            score: Jacarrd score
    '''
...
def calculate_scores( gold_span, predict_span, exact=True):
    ''' calculate_scores: calculate gold and predict scores
        parameters:
            gold_span: gold span
            predict_span: predict span
            exact: True by default, calculate both exact and inexact match results
                   else, calculate only exact results 
        return:
            exact or inexat performance (P/R/F1)   
    '''
    right = 0
    right_gold = 0
    right_predict = 0
...
def load_spans( labels, eval_type='ner' ):
    ''' load labels' spans: support eval_type as 'ner' or 'classification'. '''
    spans = {}
...
def load_combined_bio( result_file, sep_tag='\t' ):
    ''' load gold and prediction from bio file with gold and predcition together 
        parameter:
            result_file: format as TEXT GOLD_LABEL PRED_LABEL
            sep_tag: separator between columns
        return:
            gold, prediction: gold and prediction labels
    '''
    gold = []
    predict = []
    line_comments = ''
...
def evaluate(label_file, eval_score_file='', sep_tag_type='tab', eval_type='ner', exact=False, labels=None):    
    ''' evalaute for combined gold/predict label file or lables
        parameters:
            ...
            sep_tag_type: seperated tag between label file's items in each line,'tab' by default
            eval_type: 'ner' or 'classification'
